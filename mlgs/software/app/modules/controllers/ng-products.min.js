...
function inprogress($rs, $s, svc, Products){
	svc.mainMenu(function(m){
		$rs.menu = m;
		$rs.title = $rs.webtitle + " | InProgress"
	},3);
	Products.getInProgressProducts({
		limit: 4
	},function(e, r){
...
function categories($rs, $s, svc){
	svc.mainMenu(function(m){
		$rs.menu = m;
		$rs.title = $rs.webtitle + " | Categories"
	},5);
}
...
function contactus($rs, $s, svc){
	svc.mainMenu(function(m){
		$rs.menu = m;
		$rs.title = $rs.webtitle + " | ContactUs"
	},6);
}
...
function singgleProduct($rs, $s, svc, Products, $rp){
	svc.mainMenu(function(m){
		...
		title = $rp.title;
		$rs.menu = m;
		$rs.title = $rs.webtitle + " | " + title.replace('.html','');
	},2);
